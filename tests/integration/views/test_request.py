...
from adaero.models import FeedbackRequest, Period, User
...
def test_employee_able_to_send_feedback_request_within_entry_phase(
    ...
    assert invite_plain.count(app_url) > 0, invite_plain
    assert invite_plain.count(inviter.first_name) > 0, invite_plain
...
def test_employee_able_to_invite_twice(ldap_mocked_app_with_users):  # noqa: E501
    ...
    with patch("smtplib.SMTP") as _:
        ...
        invites = (
            dbsession.query(FeedbackRequest)
            .filter(
                FeedbackRequest.to_username == TEST_COMPANY_COLLEAGUE_USERNAME,
                FeedbackRequest.from_username == TEST_EMPLOYEE_USERNAME,
                FeedbackRequest.period_id == TEST_PERIOD_ID,
            )
            .all()
        )
        assert len(invites) == 1
        get_resp = app.get("/api/v1/request")
...
def test_external_user_able_to_give_and_amend_feedback_that_sent_them_invites(
    ...
    # refer to tests/integration/constants.py for full list of enrolled users
