...
def get_engine_from_config_filename(config_filename):
    ...
    if config_filename[0] == "/":
        ...
        config_uri = pkg_resources.resource_filename("adaero", config_filename)
    setup_logging(config_uri)
    settings = get_appsettings(config_uri, name="adaero", options=options)
...
def generate_revision(ctx, message):
    ...
    alembic_cfg = adaero.migration.generate_alembic_config(settings)
